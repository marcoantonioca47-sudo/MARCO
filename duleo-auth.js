...
async function loadAccess(user){
 ...
 if(pe)throw pe;if(be)throw be;if(!profile)throw new Error('Esta conta ainda não possui um perfil DULEO. Cadastre o perfil no Supabase.');
...
function wire(form,input){
 const label=input.closest('label');if(label)label.firstChild.textContent='E-mail';input.type='email';input.placeholder='seu@email.com';
 form.onsubmit=null;
 form.addEventListener('submit',async e=>{e.preventDefault();
  const email=input.value.trim().toLowerCase(),pass=document.querySelector('#loginSenha')?.value||'';if(!email||!pass){msg('Informe e-mail e senha.');return}msg('Autenticando online...');
  const {data,error}=await client.auth.signInWithPassword({email,password:pass});if(error){msg('E-mail ou senha inválidos.');return}
  try{const access=await loadAccess(data.user);if(!access.ids.length){await client.auth.signOut();msg('Sua conta não possui nenhuma filial autorizada.');return}
   const select=document.querySelector('#loginFilial'),localF=(()=>{try{return JSON.parse(localStorage.getItem('duleo_f3'))||[]}catch{return[]}})(),allowed=access.ids;
   ...
   const filial=select?.value||allowed[0];localStorage.setItem('duleo_s3',JSON.stringify({uid:data.user.id,filial}));location.reload();
