...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function numero(valor) {
        if (typeof valor === "string") {
            ...
            // Aceita números no padrão brasileiro, como 85,5 e 1.234,56.
            if (valor.includes(",")) {
    ...
    function formatarNumero(valor) {
        ...
        // Evita que valores negativos muito próximos de zero apareçam como -0.
        if (Math.abs(n) < 0.005) {
            n = 0;
        }
    ...
    function formatarPercentual(valor) {
        let n = numero(valor);
        if (!Number.isFinite(n) || Math.abs(n) < 0.005) n = 0;
    ...
    function atualizarTabelaIndicadores() {
        ...
        container.innerHTML = `
            <table class="data-table indicadores-table">
                <thead>
                    <tr>
                        <th>Indicador</th>
                        <th>Categoria</th>
                    ...
                    ${dados.map(indicador => {
                        ...
                        return `
                            <tr>
                                ...
                                <td><span class="categoria-chip">${escaparHTML(
                                    CATEGORIAS[indicador.categoria] || indicador.categoria
                                )}</span></td>

                                <td><span class="filial-chip">⌖ ${escaparHTML(filial?.nome || "-")}</span></td>

                                <td><span class="data-chip">${formatarData(indicador.data)}</span></td>

                                <td><span class="percentual-meta">${formatarNumero(indicador.meta)}%</span></td>

                                <td>
                                    <div class="resultado-cell">
                                        <strong>${formatarNumero(indicador.resultado)}%</strong>
                                        <div class="mini-progress"><span style="width:${Math.max(0, Math.min(100, numero(indicador.resultado)))}%"></span></div>
                                    </div>
                                </td>

                                <td><span class="status-pill ${status.classe}"><span class="status-pill-dot"></span>${escaparHTML(status.texto)}</span></td>
                    ...
                    return `
                        ...
                        <div class="item-recente">
                            ...
                            <div>
                                ...
                                <strong class="recente-resultado">
                                    ${formatarNumero(indicador.resultado)}%
                                </strong>
                                <div class="recente-progress"><span style="width:${Math.max(0, Math.min(100, numero(indicador.resultado)))}%"></span></div>
    ...
    function atualizarGraficoRosca(container, dados) {

        if (!container) return;

        const categorias = calcularMediasCategorias(dados);
        const total = categorias.reduce((soma, item) => {
            return soma + dados.filter(ind => ind.categoria === item.categoria).length;
        }, 0);

        const paleta = ["#3b82f6", "#22c55e", "#f59e0b", "#a78bfa"];
        let acumulado = 0;
        const fatias = categorias.map((item, index) => {
            const quantidade = dados.filter(ind => ind.categoria === item.categoria).length;
            const percentual = total ? quantidade / total * 100 : 0;
            const inicio = acumulado;
            acumulado += percentual;
            return { ...item, quantidade, percentual, inicio, fim: acumulado, cor: paleta[index] };
        });

        const gradiente = fatias.length
            ? fatias.map(item => `${item.cor} ${item.inicio}% ${item.fim}%`).join(', ')
            : '#334155 0% 100%';

        container.innerHTML = `
            <div class="rosca-layout">
                <div class="rosca" style="background:conic-gradient(${gradiente})">
                ...
                <div class="rosca-legenda">
                    ${fatias.map(item => `
                        <div class="legenda-item">
                            <span><i style="background:${item.cor}"></i>${escaparHTML(item.nome)}</span>
                            <strong>${item.quantidade} <small>(${formatarNumero(item.percentual)}%)</small></strong>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    function atualizarIndicadorGeral() {
        ...
        const mesSelecionado = $("#filtroMesGeral")?.value || mesAtualISO();
        let dados = indicadoresVisiveis().filter(indicador => {
            // Aceita datas ISO (AAAA-MM-DD), mês (AAAA-MM) e datas brasileiras.
            const data = String(indicador.data || "").trim();
            const mesIndicador = data.match(/^(\d{4})-(\d{2})/)?.[0]
                || data.match(/^(\d{2})\/(\d{2})\/(\d{4})$/)?.slice(2, 4).reverse().join("-")
                || obterMes(data);
            return mesIndicador === String(mesSelecionado).substring(0, 7);
        });

        if (ehAutor()) {
            const filial = $("#filtroFilialGeral")?.value || "todas";
            if (filial !== "todas") {
                dados = dados.filter(indicador =>
                    String(indicador.filialId) === String(filial)
                );
            }
        }
        ...
        const resultado = total
            ? dados.reduce((soma, item) => soma + numero(item.resultado), 0) / total
            : 0;
        const meta = total
            ? dados.reduce((soma, item) => soma + numero(item.meta), 0) / total
            : 0;

        $("#valorGeral").textContent = `${formatarPercentual(resultado)}%`;
        $("#geralTotal").textContent = total;
        $("#geralMeta").textContent = `${formatarPercentual(meta)}%`;
        $("#geralResultado").textContent = `${formatarPercentual(resultado)}%`;
        $("#descricaoGeral").textContent = total
        ...
        const orbital = $("#geralPercentualOrbital");
        if (progresso) progresso.style.width = `${Math.max(0, Math.min(100, resultado))}%`;
        if (orbital) orbital.textContent = `${formatarPercentual(resultado)}%`;
    ...
    function atualizarTabelaGeral(dados) {
        ...
        container.innerHTML = `
            ...
            <table class="data-table tabela-geral-dinamica">
    ...
    /* =====================================================
       EVOLUÇÃO MENSAL, BACKUP E RESTAURAÇÃO
    ===================================================== */
    function atualizarGraficoMensal() {
        const area = document.getElementById("graficoMensal");
        if (!area) return;
        const dados = Array.isArray(indicadores) ? indicadores : [];
        const meses = {};

        dados.forEach(item => {
            const data = String(item.data || item.dataIndicador || "").trim();
            let mes = data.match(/^(\d{4})-(\d{2})/)?.[0];
            if (!mes) {
                const br = data.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
                if (br) mes = `${br[3]}-${br[2]}`;
            }
            if (!mes) return;
            const valor = Math.max(0, Math.min(100, numero(item.resultado)));
            if (!meses[mes]) meses[mes] = { soma: 0, total: 0 };
            meses[mes].soma += valor;
            meses[mes].total++;
        });

        const lista = Object.keys(meses).sort().slice(-12);
        if (!lista.length) {
            area.innerHTML = '<p class="texto-auxiliar">Ainda não há dados mensais.</p>';
            return;
        }
        area.innerHTML = lista.map(mes => {
            const media = meses[mes].total ? meses[mes].soma / meses[mes].total : 0;
            const nome = new Date(`${mes}-01T12:00:00`).toLocaleDateString("pt-BR", { month: "short", year: "2-digit" });
            return `<div class="barra-item" title="${nome}: ${media.toFixed(2)}%">
                <div class="barra-valor">${media.toFixed(1).replace('.', ',')}%</div>
                <div class="barra-coluna"><span style="height:${media}%"></span></div>
    ...
    function baixarBackup() {
        const chaves = [CHAVE_INDICADORES, CHAVE_FILIAIS, CHAVE_USUARIOS, CHAVE_METAS, CHAVE_TEMA, CHAVE_SESSAO];
        const backup = { versao: "DULEO-BACKUP-1", criadoEm: new Date().toISOString(), dados: {} };
        chaves.forEach(chave => backup.dados[chave] = carregar(chave, null));
        const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `duleo-backup-${new Date().toISOString().slice(0,10)}.json`;
        a.click();
        URL.revokeObjectURL(url);
        const status = document.getElementById("statusBackup");
        if (status) status.textContent = "Backup baixado com sucesso.";
    }
    ...
    function restaurarBackup(arquivo) {
        ...
        leitor.onload = () => {
            try {
                ...
                if (!backup || backup.versao !== "DULEO-BACKUP-1" || !backup.dados) throw new Error("Formato inválido");
                ...
                Object.entries(backup.dados).forEach(([chave, valor]) => {
                    if (valor !== null && valor !== undefined) localStorage.setItem(chave, JSON.stringify(valor));
                });
                alert("Backup restaurado. O sistema será recarregado.");
                location.reload();
            } catch (erro) {
                console.error("Erro ao restaurar backup:", erro);
                alert("Não foi possível restaurar o backup. Verifique o arquivo.");
            }
        };
        leitor.readAsText(arquivo);
    }

    document.getElementById("btnExportarBackup")?.addEventListener("click", baixarBackup);
    document.getElementById("btnImportarBackup")?.addEventListener("click", () => document.getElementById("arquivoBackup")?.click());
    document.getElementById("arquivoBackup")?.addEventListener("change", evento => {
        if (evento.target.files?.[0]) restaurarBackup(evento.target.files[0]);
        evento.target.value = "";
    });
...
/* =========================================================
   DULEO 2.0 — MELHORIAS TRANSVERSAIS
   Camada visual sem alterar o modelo de dados existente.
========================================================= */
document.addEventListener("DOMContentLoaded", () => {
    const $d = (s, root=document) => root.querySelector(s);

    // Animação suave dos números dos cards e resumos.
    function animarNumero(el) {
        if (!el || el.dataset.animadoTexto === el.textContent) return;
        const texto = el.textContent.trim();
        const match = texto.match(/(-?\d+(?:[.,]\d+)?)/);
        if (!match) return;
        const alvo = Number(match[1].replace(".", "").replace(",", "."));
        if (!Number.isFinite(alvo) || Math.abs(alvo) > 1000000) return;
        const sufixo = texto.slice(match.index + match[0].length);
        const prefixo = texto.slice(0, match.index);
        const casas = match[0].includes(",") ? match[0].split(",")[1].length : 0;
        const inicio = performance.now();
        const duracao = 650;
        el.dataset.animadoTexto = texto;
        function frame(agora) {
            const p = Math.min(1, (agora - inicio) / duracao);
            const ease = 1 - Math.pow(1 - p, 3);
            const valor = alvo * ease;
            el.textContent = prefixo + valor.toLocaleString("pt-BR", {minimumFractionDigits: casas, maximumFractionDigits: casas}) + sufixo;
    ...
    const observador = new MutationObserver(mudancas => {
        mudancas.forEach(m => {
            if (m.type === "characterData" || m.type === "childList") {
                ...
                if (el && el.matches && el.matches(".card strong, #valorGeral, #geralTotal, #geralMeta, #geralResultado, #geralPercentualOrbital")) animarNumero(el);
                if (el) $$(".card strong, #valorGeral, #geralTotal, #geralMeta, #geralResultado, #geralPercentualOrbital", el).forEach(animarNumero);
            }
        });
    });
    observador.observe(document.body, {subtree:true, childList:true, characterData:true});

    // Identificação visual de atualização nas páginas.
    $$(".pagina-cabecalho").forEach(cab => {
        if (!$(".selo-atualizacao", cab)) {
            const selo = document.createElement("span");
            selo.className = "selo-atualizacao";
            selo.textContent = "● Dados em tempo real local";
            selo.title = "Os dados desta versão são atualizados no navegador.";
            cab.appendChild(selo);
        }
    });

    // Busca rápida na aba Indicadores, sem interferir nos filtros existentes.
    const tabela = $("#tabelaIndicadores");
    const cabIndicadores = $('#tela-indicadores .pagina-cabecalho');
    if (tabela && cabIndicadores && !$("#buscaRapidaIndicadores")) {
        const campo = document.createElement("input");
        campo.id = "buscaRapidaIndicadores";
        campo.type = "search";
        campo.placeholder = "Buscar indicador, categoria ou filial...";
        campo.className = "busca-rapida";
        campo.setAttribute("aria-label", "Buscar indicadores");
        cabIndicadores.appendChild(campo);
        campo.addEventListener("input", () => {
            const termo = campo.value.toLocaleLowerCase("pt-BR").trim();
            $$("#tabelaIndicadores tbody tr").forEach(tr => {
                tr.style.display = !termo || tr.textContent.toLocaleLowerCase("pt-BR").includes(termo) ? "" : "none";
            });
        });
    }
    ...
    document.addEventListener("keydown", e => {
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "k") {
            const busca = $("#buscaRapidaIndicadores");
            if (busca) { e.preventDefault(); busca.focus(); }
        }
        if (e.key === "Escape" && document.activeElement?.matches("input[type=search]")) document.activeElement.blur();
    });
});
